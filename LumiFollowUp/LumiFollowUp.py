...
#		ax_b1.set_xlabel("Bunch Slots [25ns]", fontsize=14, fontweight='bold')
	...
	def checkFiles(self, filln):
		...
		return getMassi, doSB, doSBFits, doSBModel, doLifetime, doLumiCalc, doCycle, doCycle_model, skip
	## - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - * - - *
	def runLifetime(self, filln):
		...
		for beam_n in [1, 2]:
			...
			# ###########################   CALCULATE FOR BCT READINGS
			# dNpbct 					= -(b_inten_tot[beam_n][:-1]) + (b_inten_tot[beam_n][1:])
			# tau_Np_bct[beam_n] 		= -1/((dNpbct/dt)/b_inten_tot[beam_n][:-1])
			# #Burnoff corrected lifetime from bct
			# tau_BOff_bct[beam_n] 	= b_inten_tot[beam_n]/sigmaBOff_m2/np.sum((self.filln_LumiMeasDict['ATLAS']['bunch_lumi']+self.filln_LumiMeasDict['CMS']['bunch_lumi']), axis=1)
			...
			# print dt.shape
			#
			# losses_total[beam_n]= (np.abs(dNp)/dt)/(self.filln_LumiMeasDict['ATLAS']['bunch_lumi']+self.filln_LumiMeasDict['CMS']['bunch_lumi'])[:-1]
			# losses_bct[beam_n]	= (np.abs(dNpbct)/dt)/(self.filln_LumiMeasDict['ATLAS']['bunch_lumi']+self.filln_LumiMeasDict['CMS']['bunch_lumi'])[:-1]

			dict_lifetime[beam_n]['life_time_Boff_tot'] = 1/((1/tau_Np_tot[beam_n]-1/tau_BOff_tot[beam_n][0:-1]))
			# dict_lifetime[beam_n][experiment]['life_time_Boff_bct'] = 1/((1/tau_Np_bct[beam_n]-1/tau_BOff_bct[beam_n][0:-1]))
			...
			# dict_lifetime[beam_n][experiment]['life_time_bct'] = 1/((1/tau_Np_bct[beam_n]))
			dict_lifetime[beam_n]['life_time_bbb'] = 1/((1/tau_Np_bbb[beam_n]))
			dict_lifetime[beam_n]['losses_dndtL_bbb'] = losses_bbb[beam_n]
			# dict_lifetime[beam_n][experiment]['losses_dndtL_tot'] = losses_tot[beam_n]
			# dict_lifetime[beam_n][experiment]['losses_dndtL_bct'] = losses_bct[beam_n]
	...
	def runSBModel(self, filln):
		...
		for key in self.XingAngle.keys():
			if filln in range(key[0], key[1]): # >= key[0] and filln<=key[1]:
				phi_full_rad_ATLAS	= self.XingAngle[key]
				phi_full_rad_CMS	= self.XingAngle[key]
		...
		# sigma_meas_list = []
		# sigma_meas_beam = {'beam_1':[], 'beam_2':[]}
		#
		# dt_list = []
		# fill_used_list = []
		#
		#
		# tt = (self.filln_StableBeamsDict['time_range']-self.filln_StableBeamsDict['time_range'][0])
		#
		# tot_inten_meas = np.sum(np.array(self.filln_StableBeamsDict['b_inten_interp_coll'][1])+np.array(self.filln_StableBeamsDict['b_inten_interp_coll'][2]), axis=1)/2
		#
		# tot_lumi_meas = np.sum(np.array(self.filln_LumiMeasDict['ATLAS']['bunch_lumi'])+np.array(self.filln_LumiMeasDict['CMS']['bunch_lumi']), axis=1)
		#
		# mask_remove = np.logical_or((tot_lumi_meas[:-1]+tot_lumi_meas[1:])/2.<(0.1*np.max((tot_lumi_meas[:-1]+tot_lumi_meas[1:])/2.)), tot_inten_meas[:-1]<0.1*(tot_inten_meas[:-1]))
		#
		# sigma_meas = -(np.diff(tot_inten_meas_1)/np.diff(tt))/((tot_lumi_meas[:-1]+tot_lumi_meas[1:])/2.)
		# sigma_meas = -(np.diff(tot_inten_meas_2)/np.diff(tt))/((tot_lumi_meas[:-1]+tot_lumi_meas[1:])/2.)
		#
		# sigma_meas[mask_remove]= np.nan
		# sigma_meas[sigma_meas  < sigma_min]=np.nan
		# sigma_meas[sigma_meas  > sigma_max]=np.nan
		#
		# sigma_meas_list.append(sigma_meas)
		# dt_list.append(tt[1]-tt[0])
		# fill_used_list.append(filln)
	...
	def makePerformancePlotsPerFill(self, filln, doIntLifetime, doLumiTau):     ## @TODO have not checked that yet
		...
		if doIntLifetime:
			# ## Figure Lifetime for total intensity and FBCT
			# fit_totlifetime = pl.figure('lifetime total', figsize=(15,7))
			# ax_lifetime 	= pl.subplot(2,1,1)
			# ax_lifetime2 	= pl.subplot(2,1,2, sharex=ax_lifetime, sharey=ax_lifetime)
			# # ax_lifetime3 	= pl.subplot(4,1,3, sharex=ax_lifetime, sharey=ax_lifetime)
			# # ax_lifetime4 	= pl.subplot(4,1,4, sharex=ax_lifetime, sharey=ax_lifetime)
			# ax_lifetime.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['ATLAS']['life_time_tot']/3600., 'b', 				label='ATLAS B1')
			# ax_lifetime.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['ATLAS']['life_time_tot']/3600., 'r', 				label='ATLAS B2')
			# ax_lifetime.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['CMS']['life_time_tot']/3600., 'b', ls='dotted', 		label='CMS B1')
			# ax_lifetime.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['CMS']['life_time_tot']/3600., 'r', ls='dotted', 		label='CMS B2')
			# ax_lifetime.grid('on')
			# ax_lifetime.set_xlabel('Time [h]', fontsize=14, fontweight='bold')
			# ax_lifetime.set_ylabel('Lifetime (Total)[h]', fontsize=14, fontweight='bold')
			#
			# ax_lifetime2.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['ATLAS']['life_time_Boff_tot']/3600., 'b', 	label='ATLAS B1')
			# ax_lifetime2.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['ATLAS']['life_time_Boff_tot']/3600., 'r', 	label='ATLAS B2')
			# ax_lifetime2.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['CMS']['life_time_Boff_tot']/3600., 'b', ls='dotted', 		label='CMS B1')
			# ax_lifetime2.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['CMS']['life_time_Boff_tot']/3600., 'r', ls='dotted', 		label='CMS B2')
			# ax_lifetime2.grid('on')
			# ax_lifetime2.set_xlabel('Time [h]', fontsize=14, fontweight='bold')
			# ax_lifetime2.set_ylabel('Lifetime (Total Burnoff Corrected) [h]', fontsize=10, fontweight='bold')

			# ax_lifetime3.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['ATLAS']['life_time_bct']/3600., 'b', 		label='ATLAS B1')
			# ax_lifetime3.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['ATLAS']['life_time_bct']/3600., 'r', 		label='ATLAS B2')
			# ax_lifetime3.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['CMS']['life_time_bct']/3600., 'b', ls='dotted', 		label='CMS B1')
			# ax_lifetime3.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['CMS']['life_time_bct']/3600., 'r', ls='dotted', 		label='CMS B2')
			# ax_lifetime3.grid('on')
			# ax_lifetime3.xlabel('Time [h]', fontsize=14, fontweight='bold')
			# ax_lifetime3.ylabel('Lifetime (fBCT) [h]', fontsize=14, fontweight='bold')
			#
			#
			# ax_lifetime4.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['ATLAS']['life_time_Boff_bct']/3600., 'b', 	label='ATLAS B1')
			# ax_lifetime4.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['ATLAS']['life_time_Boff_bct']/3600., 'r', 	label='ATLAS B2')
			# ax_lifetime4.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['CMS']['life_time_Boff_bct']/3600., 'b', ls='dotted', 		label='CMS B1')
			# ax_lifetime4.semilogy((time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['CMS']['life_time_Boff_bct']/3600., 'r', ls='dotted', 		label='CMS B2')
			# ax_lifetime3.grid('on')
			# ax_lifetime3.xlabel('Time [h]', fontsize=14, fontweight='bold')
			# ax_lifetime3.ylabel('Lifetime (fBCT Burnoff Corrected) [h]', fontsize=10, fontweight='bold')
			...
			# ax_b1.plot((time_range[0:-1]-time_range[0])/3600., np.mean(self.filln_LifetimeDict[1]['losses_dndtL_bbb']*1.0e31, axis=1), ls='-', lw=3, c='b', label='Beam 1')
			...
			# ax_b2.plot((time_range[0:-1]-time_range[0])/3600., np.mean(self.filln_LifetimeDict[2]['losses_dndtL_bbb']*1.0e31, axis=1), ls='-', lw=3, c='r', label='Beam 2')
			# ax_b2.axhline(80, xmin=0, xmax=1, color='black', label='$\sigma_{\mathrm{inel}}$ = 80mb')
			# ax_b2.grid('on')
			# ax_b2.set_xlabel('Time [h]', fontsize=14, fontweight='bold')
			# ax_b2.set_ylabel("$\mathbf{\left(\\frac{dN}{dt}\\right)\slash\mathcal{L}}$ [mb]", fontsize=14, fontweight='bold')
			...
			# ax_b1_bbbtau.plot((time_range[0:-1]-time_range[0])/3600., np.mean(self.filln_LifetimeDict[1]['tau_Np_bbb']/3600., axis=1), ls='-', lw=3, c='b', label='Beam 1 - $\\tau_{N_{p}^{0}}$'+'={:.2f}h'.format(np.mean(self.filln_LifetimeDict[1]['tau_Np_bbb']/3600., axis=1)[0]))
			self.plot_mean_and_spread(ax_b1_bbbtau, (time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[1]['tau_Np_bbb']/3600., label='Beam 1 - $\\tau_{N_{p}^{0}}$'+'={:.2f}h'.format(np.mean(self.filln_LifetimeDict[1]['tau_Np_bbb']/3600., axis=1)[0]), color='b', shade=True)
			...
			# ax_b2_bbbtau.plot((time_range[0:-1]-time_range[0])/3600., np.mean(self.filln_LifetimeDict[2]['tau_Np_bbb']/3600., axis=1), ls='-', lw=3, c='r', label='Beam 2 - $\\tau_{N_{p}^{0}}$'+'={:.2f}h'.format(np.mean(self.filln_LifetimeDict[2]['tau_Np_bbb']/3600., axis=1)[0]))
			self.plot_mean_and_spread(ax_b2_bbbtau, (time_range[0:-1]-time_range[0])/3600., self.filln_LifetimeDict[2]['tau_Np_bbb']/3600., label='Beam 2 - $\\tau_{N_{p}^{0}}$'+'={:.2f}h'.format(np.mean(self.filln_LifetimeDict[2]['tau_Np_bbb']/3600., axis=1)[0]), color='r', shade=True)
		...
		figlist = [fig_total, fig_int, fig_em1, fig_em2, fig_em1_raw, fig_em2_raw, fig_bl, fig_lumi_calc, fig_lumi_meas, fig_total]
		if doIntLifetime:
			# figlist.append(fit_totlifetime)
			figlist.append(fig_bbblifetime)
			figlist.append(fig_bbblosses)
			figlist.append(fig_bbb_tau)
		if doLumiTau:
		...
		if self.savePlots:
			...
			if doIntLifetime:
				figDic.update({	fig_bbb_tau           :'fill_{}_bbbIntensityTau_{}'.format(filln, saveString),
								fig_bbblifetime       :'fill_{}_bbbLifetime_{}'.format(filln, saveString),
								fig_bbblosses         :'fill_{}_bbbLosses_{}'.format(filln, saveString)})

			if doLumiTau:
				figDic.update({	fig_tauLumi_tot       :'fill_{}_TotalLumiLifetime_{}'.format(filln, saveString),
								fig_tauLumi_fit       :'fill_{}_FitLumiLifetime_{}'.format(filln, saveString)
								})
	...
	def makeCycleModelPlots(self, filln, t_start_fill, t_end_fill, t_start_STABLE, t_end_STABLE):
		...
		for interval, tag in zip(['Injection', 'he_before_SB'], ["INJ", "FT"]):
			...
			for beam_n in [1,2]:
				...
				# ax_em_h.set_xlabel('Bunch Slot [25ns]', fontsize=14, fontweight='bold')
				...
				ax_em_v.grid('on')
				tref_string = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime(t_start_STABLE))
				fig_emit.suptitle('Fill {}: STABLE BEAMS declared on {}'.format(filln, tref_string), fontsize=18)
				ax_em_v.legend(loc='best', numpoints=1)

				if config.savePlots:
					timestr  = datetime.now().strftime("%Y%m%d")
					filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
					filename = filename + "fill_{}_cycle_model_emittance_{}_{}_{}".format(filln, tag, beam_n, timestr)+self.plotFormat
					fig_emit.savefig(filename, dpi=self.plotDpi)
			...
			# ax_blen_b1.set_xlabel('Bunch Slot [25ns]', fontsize=14, fontweight='bold')
			...
			tref_string = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime(t_start_STABLE))
			fig_blen.suptitle('Fill {}: STABLE BEAMS declared on {}'.format(filln, tref_string), fontsize=18)

			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_cycle_model_bunchLength_{}_{}".format(filln, tag, timestr)+self.plotFormat
	...
	def makeSBModelPlots(self, filln, case):
		...
		for model_name in model_names:
			...
			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_sbmodel_{}_luminosity_case{}_{}".format(filln, model_name, case, timestr)+self.plotFormat
			...
			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_sbmodel_{}_emittanceColliding_case{}_{}".format(filln, model_name, case, timestr)+self.plotFormat
			...
			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_sbmodel_{}_emittanceNonColliding_case{}_{}".format(filln, model_name, case, timestr)+self.plotFormat
			...
			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_sbmodel_{}_intens_blength_Colliding_case{}_{}".format(filln, model_name, case, timestr)+self.plotFormat
			...
			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_sbmodel_{}_intens_blength_NonColliding_case{}_{}".format(filln, model_name, case, timestr)+self.plotFormat
		...
		if len(model_names)>1:
			...
			if config.savePlots:
				timestr  = datetime.now().strftime("%Y%m%d")
				filename = self.plot_dir.replace("<FILLNUMBER>", str(filln))
				filename = filename + "fill_{}_sbmodel_modelComp_luminosity_case{}_{}".format(filln, case, timestr)+self.plotFormat
	...
	def runForFill(self, filln):
		...
		getMassi, doSB, doSBFits, doSBModel, doLifetime, doLumiCalc, doCycle, doCycle_model, skip = self.checkFiles(filln)
		print '---->', getMassi, doSB, doSBFits, doSBModel, doLifetime, doLumiCalc, doCycle, doCycle_model, skip
		...
		if not self.doSBPlots and not self.doCyclePlots and not self.doCycleModelPlots and not self.doSBModelPlots:
			if not getMassi and not doSB and not doLumiCalc and not doCycle and not doCycle_model and not doSBFits and not doSBModel and not doLifetime:
				warn("#runForFill: Nothing to do for fill {}. [getMassi = {}, doSB = {}, doLumiCalc = {}, doCycle = {}, doCycle_model = {}, doSBFits = {}, doSBModel = {}, doLifetime ={},  doSBPlots = {}]".format(filln, getMassi, doSB, doLumiCalc, doCycle, doCycle_model, doSBFits, doSBModel, doLifetime, self.doSBPlots))
				return
