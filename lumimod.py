...
def main():
    ...
    parser.add_argument("-f", "--force",     help='Force a specific step [\'all\', \'cycle\', \'sb\', \'lumi\', \'massi\', \'lifetime\', \'fits\' , \'model\']', type=str)
    parser.add_argument("-d", "--doOnly",    help='Do only a specific step [\'cycle\', \'sb\', \'lumi\', \'massi\', \'model\']', type=str, default=None)
    ...
    if args.force is not None:
        if args.force == 'all':
            force = True
        elif args.force == 'cycle' or args.force == 'sb' or args.force == 'lumi' or args.force == 'massi' or args.force == 'model' or args.force == 'lifetime' or args.force == 'fits':
    ...
    if args.doOnly is not None:
        if args.doOnly == 'cycle' or args.doOnly == 'sb' or args.doOnly == 'lumi' or args.doOnly == 'massi' or args.doOnly == 'model' or args.force == 'lifetime' or args.force == 'fits':
    ...
    if doSubmit:
        ...
        fl = LumiFollowUp.LumiFollowUp(debug=debug, batch=batch, FORMAT=config.FORMAT, loglevel=loglevel, logfile=None, fills_bmodes_file=config.fills_bmodes_file,
                                       ...
                                       SB_filename=config.SB_filename, SB_fits_filename = config.SB_fits_filename, SB_models=config.models,
                       				   SB_model_filename = config.SB_model_filename , SB_burnoff_filename = config.SB_burnoff_filename, Cycle_filename=config.Cycle_filename,
                                       Cycle_model_filename=config.Cycle_model_filename, Lumi_filename=config.Lumi_filename,
                                       ...
                                       correction_factor_2h=config.correction_factor_2h, correction_factor_2v=config.correction_factor_2v,
                                       #machine parameters
                                       frev=config.frev, gammaFT=config.gammaFT, gammaFB=config.gammaFB, tauSRxy_FT=config.tauSRxy_FT, tauSRxy_FB=config.tauSRxy_FB,
                                       tauSRl_FT=config.tauSRl_FT, sigmaBOff_m2=config.sigmaBOff_m2, sigma_el_m2=config.sigma_el_m2,
