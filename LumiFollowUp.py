################################################################################
#
#   LumiFollowUp: A module than implements the creation of
...
class LumiFollowUp(object):
    def __init__(self, debug=True, batch=True,  FORMAT='%(asctime)s %(levelname)s : %(message)s',
                ...
                saveDict = True, force=False, frev = 11245.5, gamma = 6927.64, betastar_m = 0.40,
                ...
                doCyclePlots=True, doSBPlots=True, doSummaryPlots = False, doPlots = False):
        '''
    ...
    def getBmodesDF(self):
        ...
        if self.doRescale:
            ...
            bmodes['rescaledPeriod'] = bmodes['period']
    ...
    def checkFiles(self, filln):
        ...
        if os.path.exists(massi_filename):
            if self.overwriteFiles:
                warn("Dictionary Meas. Lumi pickle [{}] for fill {} already exists! Overwritting it...".format(massi_filename, filln))
                getMassi = True
            else:
                warn("Dictionary for Meas. Lumi pickle [{}] for fill {} already exists! Skipping it...".format(massi_filename, filln))
        else:
            getMassi = True
        debug('#checkFiles : Checking if Massi file [{}] for fill {} exists [{}]'.format(massi_filename, filln, (not getMassi) ))
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if os.path.exists(SB_filename):
            if self.overwriteFiles:
                warn("Dictionary SB Data pickle [{}] for fill {} already exists! Overwritting it...".format(SB_filename, filln))
                doSB = True
            else:
                warn("Dictionary SB Data pickle [{}] for fill {} already exists! Skipping it...".format(SB_filename, filln))
        else:
            doSB = True
        debug('#checkFiles : Checking if SB file [{}] for fill {} exists [{}]'.format(SB_filename, filln, (not doSB)))
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if os.path.exists(lumi_calc_filename):
            if self.overwriteFiles:
                warn("Dictionary Lumi Calc pickle [{}] for fill {} already exists! Overwritting it...".format(lumi_calc_filename, filln))
                doLumiCalc = True
            else:
                warn("Dictionary Lumi Calc pickle [{}] for fill {} already exists! Skipping it...".format(lumi_calc_filename, filln))
        else:
            doLumiCalc = True
        debug('#checkFiles : Checking if Lumi Calc file [{}] for fill {} exists [{}]'.format(lumi_calc_filename, filln, (not doLumiCalc) ))
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if os.path.exists(cycle_filename):
            if self.overwriteFiles:
                warn("Cycle pickle [{}] for fill {} already exists! Overwritting it...".format(cycle_filename, filln))
                doCycle = True
            else:
                warn("Cycle pickle [{}] for fill {} already exists! Skipping it...".format(cycle_filename, filln))
        else:
            doCycle = True
            print doCycle
        debug('#checkFiles : Checking if Cycle file [{}] for fill {} exists [{}]'.format(cycle_filename, filln, (not doCycle)  ))
...
self.Massi_filename, self.frev, self.gamma, self.betastar_m, self.crossingAngleChange, self.XingAngle, self.saveDict, self.doOnly, self.force, self.forceCycle, self.forceSB, self.forceMeasLumi, self.forceCalcLumi, self.savePlots,
self.doCyclePlots, self.doSBPlots, self.fig_tuple, self.plotFormat, self.plotDpi, self.myfontsize, self.n_skip, self.makePlotTarball)
